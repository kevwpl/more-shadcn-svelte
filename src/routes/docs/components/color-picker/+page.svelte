...
<DocPage.Root>
	...
	<DocPage.Content>
		...
		<DocPage.Example>
			<DocPage.Preview class="py-10 flex flex-col items-center gap-4">
				<ColorPicker.Root bind:value={alphaColor} allowOpacity={true} defaultFormat="rgb" />
			</DocPage.Preview>
			<DocPage.Code
...
/>`}
		...
		<DocPage.Text>The color picker works seamlessly inside popovers or dialogs.</DocPage.Text>
		<DocPage.Example>
			<DocPage.Preview class="py-10 flex flex-col items-center gap-4">
				<div class="flex items-center gap-4">
					<div
						class="w-8 h-8 rounded-full border shadow-sm"
						style:background-color={popoverColor}
					></div>
					<Popover.Root>
						<Popover.Trigger>
							{#snippet child({ props })}
