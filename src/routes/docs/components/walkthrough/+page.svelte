<script lang="ts">
	...
	const steps = [
		...
		},
		{
			...
			title: 'The Brand',
			description:
				'This component can highlight elements outside of its own component tree, like this logo in the layout.',
			position: 'right'
		},
		{
			...
			<DocPage.Preview class="py-20">
				<div class="flex flex-col items-center gap-4 text-center">
					...
					<p class="text-muted-foreground max-w-[400px]">
						Click the button below. The tour will escape this preview box and highlight the actual
						navigation sidebar and search bar of this website.
					</p>
...
<Walkthrough bind:open={openTour} {steps} padding={2}/>`}
		...
		<DocPage.Text>
			You can fully replace the default popover card with your own design by passing a snippet. The
			context object is passed to the snippet, giving you access to controls like <code>next()</code
			>
			and <code>close()</code>.
		</DocPage.Text>
		...
		<DocPage.Example>
			<DocPage.Preview class="py-20">
				<div class="flex flex-col items-center gap-4 text-center">
					...
					<p class="text-muted-foreground max-w-[400px]">
						This button triggers a tour that uses a completely custom card layout (dark mode, purple
						accents) instead of the default shadcn style.
					</p>
			...
			<DocPage.Code
				code={`<Walkthrough bind:open={open} steps={steps}>
  ...
  {/snippet}
		...
		<!-- CUSTOM INSTANCE -->
		<Walkthrough bind:open={openCustomTour} steps={customSteps}>
			{#snippet children(ctx)}
				<div
					class="bg-zinc-900 text-white p-6 rounded-2xl shadow-2xl w-[320px] border border-zinc-800 relative overflow-hidden"
				>
					<!-- Decorative blob -->
					<div
						class="absolute -top-10 -right-10 w-32 h-32 bg-purple-500/20 blur-3xl rounded-full pointer-events-none"
					></div>

					<div class="relative z-10">
						<div class="flex justify-between items-start mb-4">
							<div>
								<span class="text-xs font-mono text-purple-400 mb-1 block"
									>STEP {ctx.currentStepIndex() + 1}</span
								>
								<h3 class="font-bold text-xl tracking-tight">{ctx.currentStep()?.title}</h3>
							</div>
							<button onclick={ctx.close} class="text-white/40 hover:text-white transition-colors"
								>✕</button
							>
						</div>
						...
						<div class="flex justify-between items-center">
							<button
								...
								class="text-sm text-zinc-500 hover:text-white disabled:opacity-0 transition-colors"
							>
								Back
							</button>
