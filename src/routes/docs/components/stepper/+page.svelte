...
<DocPage.Root>
	...
	<DocPage.Content>
		...
		<DocPage.Example>
			<DocPage.Preview class="h-auto py-5">
				<div
					class="flex flex-col w-full max-w-lg mx-auto rounded-xl border bg-background shadow-sm"
				>
								...
								<Stepper.Item step={i}>
									<Stepper.Trigger step={i}>
										...
										<div class="mt-2 text-center">
											<Stepper.Title class="text-xs uppercase text-muted-foreground"
												>{s.label}</Stepper.Title
											>
										</div>
									</Stepper.Trigger>
									{#if i !== steps.length - 1}
			...
			<DocPage.Code
				code={`<Stepper.Root bind:value={currentStep}>
  ...
  {/each}
		...
		<DocPage.Text
			>Use the component without the <code>Trigger</code> wrapper to create status displays or tracking
			timelines.</DocPage.Text
		>
					...
					<Stepper.Root value={trackingStep}>
						{#each trackSteps as s, i}
							<Stepper.Item step={i}>
								...
								{#if i !== trackSteps.length - 1}
									<Stepper.Separator class="-mt-8" />
								{/if}
							</Stepper.Item>
						{/each}
					</Stepper.Root>
  ...
  <Stepper.Item step={0}>
    ...
    <Stepper.Separator />
		...
		<DocPage.Text>
			Use the <code>orientation="vertical"</code> prop. The components will automatically adjust their
			layout and separator alignment.
		</DocPage.Text>
		...
		<DocPage.Example>
			...
			<DocPage.Code
				code={`<Stepper.Root orientation="vertical">
   {#each steps as s, i}
      <Stepper.Item step={i}>
         <Stepper.Trigger step={i}>
            <Stepper.Indicator step={i} />
				...
				<div class="w-full max-w-xl">
					...
					<div class="flex justify-center mt-8 gap-4">
						<Button
							variant="outline"
							size="sm"
							onclick={() => (customStep = Math.max(0, customStep - 1))}>Prev</Button
						>
						<Button
							variant="outline"
							size="sm"
							onclick={() => (customStep = Math.min(2, customStep + 1))}>Next</Button
						>
					</div>
				</div>
			</DocPage.Preview>
			<DocPage.Code
				code={`<Stepper.Indicator step={i} class="h-12 w-12 rounded-lg">
